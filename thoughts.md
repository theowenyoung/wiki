...
- 纯文本的工具应该是辅助性输入的。
- 手机可能是人类最有害的发明。实在是让你太方便被之吸取注意力了。我在大多数方面都是自由主义，但是这件事，也许国家应该出台政策，就像电影分级，吸烟一样，由第三方机构给注意力吸取的网站/服务分出类，然后作出一定限制，比如说限制他们不要用算法，限制他们不要用无线滚动，限制人们每天只能在这个网站上待 30 分钟，就强制退出。或者前期先行鼓励手机厂商生产只有简单功能的手机，这个手机没有浏览器，只能每小时同步一次信息，为了简化手机的开发功能，该手机被设计为只支持[Gemini](https://gemini.circumlunar.space/)协议，这是一个简单的文本协议，用很少的代码就可以渲染出来，然后还可以配个墨水屏。该手机可以给电脑传输信息，同样也是每小时一次，为了简化开发，只允许传纯文本消息，给某个邮件地址，一次只能传一份东西，不要考虑列表。这个手机的功能如下：1 个电话，1 个短信，一个每天只能同步 1 次的 RSS 客户端，这个客户端对内容源也有要求，内容源被设计为每天只能推送 1 则信息， 每月可以紧急推送 4 次。RSS 可以被设计为 web3 的形式，允许内容源做付费订阅。该手机可以生成多个加密钱包地址。系统内置加密钱包 id，且可以随机生成多个。 - 2022.03.19
